...
function url_query_string_value(str) {
    return new URL(window.location.href).searchParams.get(str);
}

let url_backwards = function(localhost) {
...
function auto_grow(element, min, max) {
...
function scroll_to(child, behavior) {
...
function Request(address, formdata) {
    return new Promise((resolve, reject) => {
...
function choose_file(attr, chosen) {
    attr = attr ? attr : {};
    return new Promise((resolve, reject) => {
        if (chosen) {
            resolve([chosen])
...
function collectInfo(maintitle, collections, maindescription = '') {
